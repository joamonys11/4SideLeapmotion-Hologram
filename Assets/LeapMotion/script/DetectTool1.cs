...
public class DetectTool1 : Detector {
	...
	void Awake () {
		...
		if (Application.isPlaying) {
			...
			if (sen != null) {
				...
				sen.text = loadfinger [9].ToString ()+" : R";
			...
			if (loadfinger [0].ToString () == "Extended") {
				Thumb = DetectTool1.PointingState.Extended;
				//Debug.Log ("Do Extend");
			} else if (loadfinger [0].ToString () == "NotExtended") {
				Thumb = DetectTool1.PointingState.NotExtended;

			} else
				Thumb = DetectTool1.PointingState.Either;



			if (loadfinger [1].ToString () == "Extended") {
				Index = DetectTool1.PointingState.Extended;
				//Debug.Log ("Do Extend");
			} else if (loadfinger [1].ToString () == "NotExtended") {
				Index = DetectTool1.PointingState.NotExtended;

			} else
				Index = DetectTool1.PointingState.Either;


			if (loadfinger [2].ToString () == "Extended") {
				Middle = DetectTool1.PointingState.Extended;
				//Debug.Log ("Do Extend");
			} else if (loadfinger [2].ToString () == "NotExtended") {
				Middle = DetectTool1.PointingState.NotExtended;

			} else
				Middle = DetectTool1.PointingState.Either;



			if (loadfinger [3].ToString () == "Extended") {
				Ring = DetectTool1.PointingState.Extended;
				//Debug.Log ("Do Extend");
			} else if (loadfinger [3].ToString () == "NotExtended") {
				Ring = DetectTool1.PointingState.NotExtended;

			} else
				Ring = DetectTool1.PointingState.Either;



			if (loadfinger [4].ToString () == "Extended") {
				Pinky = DetectTool1.PointingState.Extended;
				//Debug.Log ("Do Extend");
			} else if (loadfinger [4].ToString () == "NotExtended") {
				Pinky = DetectTool1.PointingState.NotExtended;

			} else
				Pinky = DetectTool1.PointingState.Either;
	...
	void Update () {
		...
		if (frame.Hands.Count > 0 ) {
			...
			if (Cap2.activeInHierarchy) {
				List<Hand> hands = frame.Hands;
				hand = hands [0];
				fingers = hand.Fingers;	
			} else if (Cap1.activeInHierarchy) {
		...
		//Debug.Log ("Palm Velocity Y " + hand.PalmVelocity.y);
