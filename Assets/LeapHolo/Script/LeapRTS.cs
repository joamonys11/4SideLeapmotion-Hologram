...
//      }
      ...
      GameObject pinchControl = new GameObject("RTS Anchor");
			//pinchControl.transform.position = new Vector3 (0, 20f, 0);
      _anchor = pinchControl.transform;
      _anchor.transform.parent = transform.parent;
      transform.parent = _anchor;

			scale = _anchor.localScale;
    ...
    private void transformDoubleAnchor() {
      ...
      if (_allowScale) { 
				...
				print ("Anchor  :" + _anchor.localScale);

		


				if (_anchor.localScale.x < 55f  ) 
				{
					_anchor.localScale = Vector3.one * Vector3.Distance (_pinchDetectorA.Position, _pinchDetectorB.Position);
						//_anchor.localScale = new Vector3 (68f, 68f, 68f);


						print ("Anchor  :" + _anchor.localScale);
						//if(_anchor.localScale.)
				}
